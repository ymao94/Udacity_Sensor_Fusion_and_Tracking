...
def show_pcl(pcl):
    ...
    def callback(vis):
        vis.close()
    vis.register_key_callback(key=262, callback_func=callback)
    vis.update_renderer()
    vis.poll_events()
    vis.run()
...
def bev_from_pcl(lidar_pcl, configs):
    ...
    lidar_cpy = np.copy(lidar_pcl)
    lidar_cpy[:,0] = np.int_(np.floor(lidar_cpy[:,0] / bev_disc))
    # step 3 : perform the same operation as in step 2 for the y-coordinates but make sure that no negative bev-coordinates occur
    lidar_cpy[:, 1] = np.int_(np.floor(lidar_cpy[:, 1] / bev_disc) + (configs.bev_width + 1) / 2)
    lidar_cpy[:, 1] = np.abs(lidar_cpy[:, 1])
    ...
    lidar_cpy[lidar_cpy[:, 3] > 1.0, 3] = 1.0
    indices = np.lexsort((-lidar_cpy[:, 3], lidar_cpy[:, 2], lidar_cpy[:, 1]))
    lidar_pcl_top = lidar_cpy[indices]
    ...
    _, _, counts = np.unique(lidar_cpy[:, 0:2], axis=0, return_index=True, return_counts=True)
